...
data DimArray e where {
    DimArray ::(Storable e, Num e) => ArrayMeta -> ForeignPtr e -> DimArray e
  } deriving (Typeable)

arrayMeta :: (Storable e, Num e) => DimArray e -> ArrayMeta
arrayMeta (DimArray !meta _) = meta

arrayFlatVector :: (Storable e, Num e) => DimArray e -> I.Vector e
arrayFlatVector (DimArray (ArrayMeta _ !shape _) !fptr) =
  I.unsafeFromForeignPtr fptr 0 (dbArraySize shape)

arrayFlatMVector
  :: (Storable e, Num e) => DimArray e -> M.MVector (PrimState IO) e
arrayFlatMVector (DimArray (ArrayMeta _ !shape _) !fptr) =
  M.unsafeFromForeignPtr fptr 0 (dbArraySize shape)


-- | unwrap an array from Edh object form
unwrapArrayObject
  :: forall e . (Typeable e) => Object -> STM (Maybe (DimArray e))
...
mmapArray
  :: forall e . (Storable e, Num e) => Text -> ArrayMeta -> IO (DimArray e)
...
type DbF8Array = DimArray Double
type DbF4Array = DimArray Float
type DbI8Array = DimArray Int64


-- | host constructor DimArray(dataDir, dataPath, shape, dtype='f8', len1d=0)
aryHostCtor
  :: EdhProgState
          ...
          Nothing -> case fromDynamic esd of
            Nothing ->
              throwEdhSTM pgs UsageError
                $  "bug: this is not an ary : "
                <> T.pack (show esd)
            Just ((DimArray (ArrayMeta !path !shape !len1d) _) :: DbI8Array) ->
              exitEdhSTM pgs exit
                $  EdhString
                $  "db.Array("
                <> T.pack (show path)
                ...
                <> T.pack (show len1d)
                <> ")"
          Just ((DimArray (ArrayMeta !path !shape !len1d) _) :: DbF4Array) ->
            exitEdhSTM pgs exit
              $  EdhString
              $  "db.Array("
              <> T.pack (show path)
              ...
              <> T.pack (show len1d)
              <> ")"
        Just ((DimArray (ArrayMeta !path !shape !len1d) _) :: DbF8Array) ->
          exitEdhSTM pgs exit
            $  EdhString
            $  "db.Array("
            <> T.pack (show path)
