...
floatOperations = FloatOp newPiArray
                          exp'op
                          log'op
                          sqrt'op
                          sin'op
 ...
 where
  newPiArray !cap !exit = (exit =<<) $ unsafeIOToSTM $ do
    !p  <- callocArray @a cap
...
resolveFloatDataOperator' !ets !dti !naExit !exit =
  runEdhTx ets
    $ performEdhEffect (AttrBySym resolveFloatDataOperatorEffId)
                       [EdhString dti]
                       []
    $ \case
        EdhNil           -> const naExit
        ...
        DeviceArray !cap !fp ->
          resolveFloatDataOperator ets (data'type'identifier dt) $ \ !fo ->
            case fromDynamic $ fop fo of
              ...
              Just (op :: a -> a) -> do
                !rfa <- unsafeIOToSTM $ withForeignPtr fp $ \(p :: Ptr a) -> do
                  !rp  <- callocArray cap
